  ...
  <body class="h-full bg-gray-100 dark:bg-gray-900 transition-colors">
    ...
    <div class="flex min-h-full flex-col justify-center px-6 py-12 lg:px-8">
      ...
      <div class="sm:mx-auto sm:w-full sm:max-w-sm text-center">
        ...
        <h2 id="loginTitle" class="text-2xl font-bold text-gray-900 dark:text-white tracking-tight">
          Login UMKM
        </h2>
        <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
          Belum punya akun?
          <a id="registerLink" href="{{ route('authenticate.register') }}"
      ...
      <div class="mt-8 sm:mx-auto sm:w-full sm:max-w-sm">
        ...
        <form class="space-y-5" method="POST" action="{{ route('authenticate.login.post') }}" onsubmit="showLoading()">
          @csrf
          ...
          <div>
            <div class="flex items-center justify-between mb-1.5">
              ...
              <a href="#" class="text-xs font-medium text-green-600 hover:text-green-500 transition">Lupa password?</a>
            </div>
            <div class="relative">
              <span class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400">
                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round">
              ...
              <button type="button" onclick="togglePw('pwInput')"
                class="eye-btn absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300">
                <svg id="eyeOpen" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round">
                  <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/>
                </svg>
                <svg id="eyeClosed" class="hidden" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round">
          ...
          <div class="flex items-center gap-2">
            <input id="remember" type="checkbox" name="remember"
              class="w-4 h-4 rounded border-gray-300 text-green-600 focus:ring-green-500 cursor-pointer accent-green-600" />
          ...
          <button type="submit" id="submitBtn"
            class="w-full bg-green-600 hover:bg-green-700 active:scale-[.98] text-white font-semibold py-2.5 rounded-lg transition-all flex items-center justify-center gap-2 text-sm shadow-sm shadow-green-300 dark:shadow-none">
    ...
    <script>
      ...
      function switchRole(role) {
        ...
        document.getElementById('loginTitle').textContent = isUMKM ? 'Login UMKM' : 'Login Investor';
      }
      ...
      // Menangkap session sukses (opsional jika redirect balikan ke halaman ini lagi)
      @if (session('success'))
