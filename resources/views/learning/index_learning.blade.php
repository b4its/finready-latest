...
<div class="layout">
  <main class="main-col">
    ...
    @php
        ...
        $moduleProgressPct = $moduleContentsCount > 0 ? round(($completedInModule / $moduleContentsCount) * 100) : 0;
    ...
    <div class="module-wrap">
      ...
      <div class="module-card {{ $modStatusClass }}" id="mc{{ $module->id }}">
        ...
        <div class="module-progress-bar">
          ...
          <div class="mini-bar-bg"><div class="mini-bar-fill" style="width:{{ $moduleProgressPct }}%"></div></div>
        </div>
        ...
        <div class="lessons-list" id="ll{{ $module->id }}">
          ...
          @empty
            @if($module->rooms->isEmpty())
                <div class="lesson-item" style="pointer-events: none;">
  ...
  <aside class="sidebar-col">
    <div class="card">
      ...
      <div class="card-body">
        ...
        <div class="stat-row">
          <div class="stat-icon highlight"><svg fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M11.48 3.499a.562.562 0 0 1 1.04 0l2.125 5.111a.563.563 0 0 0 .475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 0 0-.182.557l1.285 5.385a.562.562 0 0 1-.84.61l-4.725-2.885a.562.562 0 0 0-.586 0L6.982 20.54a.562.562 0 0 1-.84-.61l1.285-5.386a.562.562 0 0 0-.182-.557l-4.204-3.602a.562.562 0 0 1 .321-.988l5.518-.442a.563.563 0 0 0 .475-.345L11.48 3.5Z" /></svg></div>
          <div><div class="stat-point-text">{{ number_format($totalPoints) }} Poin</div><div class="stat-sub">Total Diperoleh</div></div>
        </div>

        <div class="stat-row">
          <div class="stat-icon"><svg fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 6.042A8.967 8.967 0 0 0 6 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 0 1 6 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 0 1 6-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0 0 18 18a8.967 8.967 0 0 0-6 2.292m0-14.25v14.25"/></svg></div>
          <div><div class="stat-label">{{ $totalLessons ?? 0 }} Materi</div><div class="stat-sub">{{ $totalModules }} modul</div></div>
        </div>
        ...
        <div class="stat-row">
          ...
          <div><div class="stat-label">{{ $completedModulesCount }} dari {{ $totalModules }}</div><div class="stat-sub">Modul selesai</div></div>
        </div>
      </div>
    </div>
    ...
    <div class="card">
      ...
      <div class="card-body">
        <div class="instructor-row">
          ...
          <div><div class="stat-label">{{ $user_account->name ?? 'Admin Sistem' }}</div><div class="stat-sub">{{  $user_account->role ?? 'Mentor'  }}</div></div>
        </div>
      </div>
    </div>
  </aside>
